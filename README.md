...
| Real star catalog (PTSKCAT0) load + magnitude/visibility filtering | Working | `:core:catalog`, see `docs/real_star_visibility_contract.md` |
...
| Calibrated analysis-buffer camera intrinsics (Camera2 → `ImageAnalysis` buffer) | Implemented, blocked | CAM-2c; hits `UnsupportedLogicalMultiCameraMapping` on logical multi-camera devices (confirmed on a real Pixel 9's rear camera); falls back to the legacy path. See `docs/camera_coordinate_calibration_contract.md` |
...
The astronomy work here decomposes into three pieces that don't depend on
each other, which is why they can be built, tested, and validated
independently.
...
`:core:astro`. Full build/calibration procedure: `res/skyglow/REAL_GRID_RUNBOOK.md`.

**Spectrum** — converting a Bortle/SQM/limiting-magnitude sky-quality input
into which catalog stars are visible, independent of how that sky-quality
number was obtained (grid lookup or manual entry). Implemented as
`RealStarVisibilityService` in `:core:catalog` over the PTSKCAT0 real star
catalog. Contract: `docs/real_star_visibility_contract.md`; binary format:
`docs/star_catalog_ptskcat0_format.md`.

**On-device camera** — turning a live camera frame into identified stars:
capture geometry (CAM-1) → pixel-level detection (SKY-2) → predicted
catalog-star positions from pose (CAM-2a/2b) → calibrated intrinsics
(CAM-2c, blocked on logical multi-camera devices) → matching detections to
catalog identities (not yet implemented). Contracts:
...
`app/` and `store/` exist at the repository root but are not included in
`settings.gradle.kts` — `app/` is not part of the active build graph, and
`store/` holds Play Store listing assets (screenshots, icons, disclaimers),
not code.
