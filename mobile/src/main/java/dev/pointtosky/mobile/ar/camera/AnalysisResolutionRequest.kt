 ...
 * `analysisResolutionOverride` parameter carries it — the same shared-type-with-debug-only-caller
 * pattern as `cameraSelectorOverride`; no production call site ever constructs one.
 *
 ...
 */
enum class AnalysisResolutionFamily {
 ...
 */
data class AnalysisResolutionRequest(
    val widthPx: Int,
    val heightPx: Int,
 ...
 * `WxH` is an exact 16:9/4:3 ratio.
 */
fun aspectRatioStrategyFor(family: AnalysisResolutionFamily): AspectRatioStrategy =
    when (family) {
