 ...
 */
object AimSender {
    fun sendEquatorial(bridge: MobileBridge.Sender, raDeg: Double, decDeg: Double): MobileBridge.Sender.Ack? {
        return bridge.send(PATH_AIM_SET_TARGET) { cid ->
            val msg = AimSetTargetMessage(
                cid = cid,
    ...
    fun sendBody(bridge: MobileBridge.Sender, bodyName: String): MobileBridge.Sender.Ack? {
        return bridge.send(PATH_AIM_SET_TARGET) { cid ->
            val msg = AimSetTargetMessage(
                cid = cid,
    ...
    fun sendStar(bridge: MobileBridge.Sender, id: Int, raDeg: Double, decDeg: Double): MobileBridge.Sender.Ack? {
        return bridge.send(PATH_AIM_SET_TARGET) { cid ->
            val msg = AimSetTargetMessage(
                cid = cid,
