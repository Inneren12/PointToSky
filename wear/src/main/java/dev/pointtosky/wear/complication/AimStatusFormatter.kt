...
class AimStatusFormatter(context: Context) {
    ...
    private val degreeFormat: NumberFormat = NumberFormat.getNumberInstance(locale).apply {
        maximumFractionDigits = 1
        minimumFractionDigits = 0
        isGroupingUsed = false
    }

    fun shortText(deltaAzDeg: Double?, deltaAltDeg: Double?): String? {
        val az = deltaAzDeg?.let { formatAz(it) }
        val alt = deltaAltDeg?.let { formatAlt(it) }
        return if (az != null && alt != null) "$az $alt" else null
    }

    fun phaseLabel(phase: AimPhase?): String = when (phase) {
    ...
    fun descriptionComponent(deltaDeg: Double?, axis: Axis): String? {
        val value = deltaDeg ?: return null
        val directionRes = when (axis) {
            Axis.HORIZONTAL -> when {
                value > thresholdDeg -> R.string.comp_aim_status_direction_right
                value < -thresholdDeg -> R.string.comp_aim_status_direction_left
                else -> R.string.comp_aim_status_direction_center
            }
            Axis.VERTICAL -> when {
                value > thresholdDeg -> R.string.comp_aim_status_direction_up
                value < -thresholdDeg -> R.string.comp_aim_status_direction_down
                else -> R.string.comp_aim_status_direction_level
            }
        }
        val magnitude = degreeFormat.format(abs(value))
        return resources.getString(R.string.comp_aim_status_offset_template, magnitude, resources.getString(directionRes))
    }
    ...
    companion object {
        ...
        private const val thresholdDeg = 0.2
    }
